...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-07-01 15:12:35 | RBA-RBR non-GPU coordinate/budget audit CLI | `tools/rba_rbr/audit_coordinate_budget.py`, `tests/test_rba_rbr_diagnostics.py`, route docs only | Local CPU diagnostic tooling complete | Self-check and focused local tests passed; Pro gate not waived; formal/full RBA-RBR train remains locked | CLI reads grid-audit JSONL or synthetic rows, summarizes `raw_valid_k`, detector valid count, raw/detector gaps, flags detector density far below 384/192 50% reference, and runs synthetic coordinate closure without validation/test GT; no GPU, remote sync, Slurm, `tools/test.py`, mAP/runtime/FLOPs/deploy/paper claim, or full-train unlock | Use output to plan `postprocess-guard shortdiag`, matched low-budget uniform, `K=192` forced coverage, and coordinate-closure Pro diagnosis; do not launch long follow-up or route conclusion without valid GPT-5.5 Pro diagnosis or explicit same-scope user override |
| 2026-07-01 14:47:42 | RBA-RBR corrected guard diagnostic final result, child `1118197.560` | Read-only result/evidence documentation only | Completed diagnostic, severe-low final | Severe-result gate remains active; formal/full RBA-RBR train locked; no claim unlocked; Pro diagnosis required before long follow-up or route conclusion | Logdir `/data/run01/sczc063/yuzibo/OpenTAD_RBA_RBR_GuardDiag_20260701_e6de60e9_bundle/logs/rba_rbr_guard_evaldiag_manual_parallel_holdg0_20260701_123213_+0800`; Average-mAP lines: `0.22%` at `13:39:19`, final `7.45%` at `14:47:42`; final vector `17.34/11.47/5.69/2.09/0.65`; `411700` predictions, `3325` GT; `Training Over` count `1`, hard error count `0`; guard audit `4090` rows all `PASS_RBA_RBR_NATIVE_AXIS_POSITIONS_ENTERED_MODEL`, `raw_valid_k` avg `84.6235`, `mask_true_count` avg `42.5601`, selected gap max/avg `16/15.5897`, detector gap max/avg `24/22.5460`; latest GitHub branch `codex/divergent-rba-rbr-guard-complete-39073521-20260701` | Preserve as severe-result evidence; send complete packet to GPT-5.5 Pro when available; do not launch formal/full train, long follow-up, or route conclusion without valid Pro diagnosis or explicit same-scope user override |
| 2026-07-01 14:18:00 | RBA-RBR postprocess guard GitHub evidence sync | GitHub API branch sync only | Completed sync | Ordinary `git push` failed with network reset; GitHub Git Data API sync succeeded without force push | Local commit `546ed615`; GitHub branch `codex/divergent-rba-rbr-postprocess-guard-546ed615-20260701`, ref `6ba723abe4f5b2eca7c38f71c5b14872d637dbaa`, URL `https://github.com/yuzbo/pc-ot-mras-r-series-opentad/tree/codex/divergent-rba-rbr-postprocess-guard-546ed615-20260701`; synced 8 RBA postprocess-guard files | Use GitHub URL for Pro/severe-result review and route-owned remote sync; no formal/full train or claims |
| 2026-07-01 14:09:53 | RBA-RBR postprocess candidate guard repair | `IrregularActionFormer` RBA-specific postprocess guard/audit, RBA main config, RBA launch gate, focused tests | Local repair complete, not deployed | Local self-check passed; Pro/Rosetta remains unavailable from prior transport evidence; severe-result gate still locks formal/full train | Added `rba_rbr_postprocess_guard` with fail-closed RBA metadata requirement and caps `raw_proposal_cap=1024`, `per_class_topk=32`, `total_candidate_cap=512`; local py_compile passed; `pytest tests/test_rba_rbr_integration.py -q` -> `8 passed, 7 skipped`; synthetic ledger + launch gate -> `gate_pass=true`; no remote sync/Slurm/new mAP yet | If `.560` finishes severe-low, sync this route-owned repair for a new `SHORT_DIAGNOSTIC_ONLY` postprocess-guard run; do not launch formal/full training or claims without valid Pro/user override |
| 2026-07-01 13:39:19 | RBA-RBR corrected guard diagnostic first validation | Read-only result collection and guard grid-audit summary | Running, severe-low first validation | Severe-result gate remains active; Pro/Rosetta unavailable (`9223/9333/9222` refused, Oracle API key missing); formal/full train locked | Child `1118197.560 rba_guard_g0` on protected hold GPU0 reached first validation: `Average-mAP=0.22%`, vector `0.66/0.30/0.09/0.04/0.01`, `3325` GT, `411700` predictions; no Traceback/OOM/NaN/non-finite observed; guard audit `2183` rows, all route label/status PASS, `mask_true_count` avg `42.53`, `selected_max_gap_after_guard` max `16`, `max_detector_gap_after_guard` max `24`, only `1` row below detector feature floor | Let this short diagnostic continue to scheduled epoch-3/final only; do not launch RBA-RBR formal/full train or new long follow-up until severe-result diagnosis is complete |
| 2026-07-01 12:34:06 | RBA-RBR corrected guard diagnostic, `input_rba_rbr_recoverable_bracketing_adapter_irregular_headv3_evaldiag.py` | Remote orchestration only; no model/config code change beyond existing guard branch `e6de60e9` | Running on protected hold GPU0 as short diagnostic | Full train locked; public pending job cancelled only after exact `1132718|rba_guarddiag|PENDING` recheck; watcher PID `1840205` stopped to avoid duplicate launch | Child step `1118197.560 rba_guard_g0` running in parallel with BVR `1118197.542`; logdir `/data/run01/sczc063/yuzibo/OpenTAD_RBA_RBR_GuardDiag_20260701_e6de60e9_bundle/logs/rba_rbr_guard_evaldiag_manual_parallel_holdg0_20260701_123213_+0800`; startup passed `CUDA_VISIBLE_DEVICES_INITIAL=0`, torch distributed init, finite losses through `[000][00040/00199] Loss=2.5124`, and `46` grid-audit rows; no Traceback/OOM/NaN observed | Monitor to completion; summarize guard audit counts/gaps and diagnostic mAP; no formal/full train or claims until reviewed |
...
### 2026-07-01 15:12:35 +08:00 - RBA-RBR non-GPU coordinate/budget audit CLI

- Route label: `DIVERGENT_INNOVATION_RBA_RBR_DO_NOT_MERGE_WITH_C3`.
- Added CPU-only diagnostic tooling in `tools/rba_rbr/audit_coordinate_budget.py` plus focused tests in `tests/test_rba_rbr_diagnostics.py`.
- Diagnostic scope: consume RBA-RBR grid-audit JSONL or synthetic rows; summarize raw valid count, detector valid count, raw/detector gap stats, and flag when effective detector density is far below the fixed 384/192 50% reference.
- Coordinate closure: synthetic raw selected native-axis positions are grouped into detector feature centers and checked against synthetic native-axis segments; `uses_validation_or_test_gt=False`.
- Verification: `python -m py_compile tools\rba_rbr\audit_coordinate_budget.py tests\test_rba_rbr_diagnostics.py` passed; `python -m pytest tests\test_rba_rbr_diagnostics.py -q` -> `5 passed`.
- Decision: no GPU, remote sync, Slurm, `tools/test.py`, evaluator/postprocess runtime change, metric/runtime/FLOPs/deploy/paper claim, or full-train unlock. Use this only to structure next diagnostics: postprocess-guard shortdiag, matched low-budget uniform, `K=192` forced coverage, and coordinate-closure Pro review.

### 2026-07-01 14:47:42 +08:00 - RBA-RBR guard diagnostic completed severe-low

- Route label: `DIVERGENT_INNOVATION_RBA_RBR_DO_NOT_MERGE_WITH_C3`.
- Run: protected-hold child `1118197.560 rba_guard_g0`, logdir `/data/run01/sczc063/yuzibo/OpenTAD_RBA_RBR_GuardDiag_20260701_e6de60e9_bundle/logs/rba_rbr_guard_evaldiag_manual_parallel_holdg0_20260701_123213_+0800`.
...
- Guard audit: `4090` rows, all `PASS_RBA_RBR_NATIVE_AXIS_POSITIONS_ENTERED_MODEL`; `raw_valid_k` avg `84.6235`; `mask_true_count` avg `42.5601`; `selected_max_gap_after_guard` max `16`, avg `15.5897`; `max_detector_gap_after_guard` max `24`, avg `22.5460`.
...
- Decision: formal/full RBA-RBR train remains locked; no metric/runtime/FLOPs/sparse-compute/deploy/paper claim and no route conclusion is unlocked. A valid GPT-5.5 Pro severe-result diagnosis is required before any long follow-up or route-level conclusion.

### 2026-07-01 14:18:00 +08:00 - RBA-RBR postprocess guard GitHub evidence branch synced

- Ordinary `git push` failed with `Recv failure: Connection was reset`.
- Created GitHub evidence branch from prior severe-firsteval branch without force push: `codex/divergent-rba-rbr-postprocess-guard-546ed615-20260701`.
- GitHub API sync used Git Data API blobs/trees/commits because the Contents API returned 404 for current branch refs.
- Local commit: `546ed6159c2155633981050ae102547de03ea6bc`.
- GitHub base commit: `1cbdb50422f9b1c968ef92e9c036d2787d434c79`.
- GitHub synced commit/ref: `6ba723abe4f5b2eca7c38f71c5b14872d637dbaa`.
...
- Synced files: RBA main config, `IrregularActionFormer`, RBA launch gate, RBA integration tests, tracker mirror, route log, severe-low diagnosis packet, and RBA local implementation report.
- This is evidence synchronization only. It does not provide a Pro verdict, remote precheck, Slurm launch, training result, `tools/test.py`, final mAP, runtime/FLOPs, sparse-compute, deploy, paper claim, or formal/full-train unlock.

### 2026-07-01 14:09:53 +08:00 - RBA-RBR postprocess candidate guard local repair

- Trigger: corrected coverage-guard diagnostic child `1118197.560 rba_guard_g0` reached first validation with `Average-mAP=0.22%` and `411700` predictions while grid-audit coverage/gap constraints were restored. This points away from simple "too few points / large holes" and toward uncontrolled candidate generation or downstream sparse-to-detector semantics.
- Changed files: `opentad/models/detectors/irregular_actionformer.py`, `configs/adatad/thumos/input_rba_rbr_recoverable_bracketing_adapter_irregular_headv3.py`, `tools/rba_rbr/validate_rba_rbr_launch_gate.py`, `tests/test_rba_rbr_integration.py`.
- Behavior: added a route-specific `rba_rbr_postprocess_guard` path. When enabled, it fails closed unless the sample has RBA-RBR metadata, caps raw proposals by deploy-visible model scores, applies per-class top-k, applies a total candidate cap, and records `RBA_RBR_POSTPROCESS_AUDIT_PATH` JSONL rows labeled `DIVERGENT_INNOVATION_RBA_RBR_DO_NOT_MERGE_WITH_C3`.
- Config/gate: RBA main config now sets `pre_nms_topk=512`, `raw_proposal_cap=1024`, `per_class_topk=32`, `total_candidate_cap=512`, `min_score=0.001`; the RBA launch gate requires these fields.
- Verification: py_compile passed for changed Python/config/test files; `python -m pytest tests/test_rba_rbr_integration.py -q` -> `8 passed, 7 skipped`; synthetic ledger build and `validate_rba_rbr_launch_gate.py` -> `gate_pass=true`, `full_train_unlocked=false`; `git diff --check` passed with LF/CRLF warnings only.
- Decision: local repair is ready for route-owned sync and another `SHORT_DIAGNOSTIC_ONLY` run only after current `.560` evidence is harvested or stopped by a concrete hard failure. Formal/full RBA-RBR training, final mAP, runtime/FLOPs, sparse-compute, deploy, and paper claims remain locked pending valid Pro or explicit user override.

### 2026-07-01 13:39:19 +08:00 - RBA-RBR guard diagnostic first validation severe-low

- Route label: `DIVERGENT_INNOVATION_RBA_RBR_DO_NOT_MERGE_WITH_C3`.
- Run: protected-hold child `1118197.560 rba_guard_g0`, logdir `/data/run01/sczc063/yuzibo/OpenTAD_RBA_RBR_GuardDiag_20260701_e6de60e9_bundle/logs/rba_rbr_guard_evaldiag_manual_parallel_holdg0_20260701_123213_+0800`.
- First validation after epoch 1 completed at `2026-07-01 13:39:19 +08:00`: `Average-mAP=0.22%`, with `mAP@0.3/0.4/0.5/0.6/0.7 = 0.66/0.30/0.09/0.04/0.01`; evaluator saw `3325` GT instances and `411700` predictions.
- Health evidence: training continued into epoch 2 with finite losses (`[002][00120/00199] Loss=1.6674 ... mem=9344MB` at latest inspection); no Traceback/OOM/NaN/non-finite pattern was observed.
- Guard audit at inspection time: `2183` JSONL rows, all `route_label=DIVERGENT_INNOVATION_RBA_RBR_DO_NOT_MERGE_WITH_C3` and `status=PASS_RBA_RBR_NATIVE_AXIS_POSITIONS_ENTERED_MODEL`; `raw_valid_k` avg `84.56`; `mask_true_count` avg `42.53`, min `30`, p50 `43`, p95 `46`; `selected_max_gap_after_guard` max `16`; `max_detector_gap_after_guard` max `24`; `guard_addition_count` avg `29.88`; `mask_lt32=1`, `rawgap_gt16_after=0`, `detgap_gt24_after=0`.
- Interpretation: the coverage guard restored detector feature count and gap constraints, but the metric still collapsed at first validation. The remaining failure is therefore not simply "large holes" or "too few detector feature positions".
- Pro gate state: `rosetta probe --port 9223 --host 127.0.0.1`, `9333`, and `9222` all refused connection; `oracle doctor --providers --models gpt-5-pro,gpt-5.5-pro,gpt-5.4-pro` reported OpenAI key missing. This is `INCOMPLETE/UNAVAILABLE`, not a valid Pro decision.
...
- Decision: keep the current run as `SHORT_DIAGNOSTIC_ONLY` and allow it to reach scheduled epoch-3/final diagnostic if it stays finite. Do not launch RBA-RBR formal/full training, do not start another RBA long follow-up, and do not make mAP/runtime/FLOPs/sparse-compute/deploy/paper claims until severe-result diagnosis is complete.
- Resource boundary: parent hold `1118197 pcot_dbg2g` was not released, cancelled, replaced, or modified; GPU1/C3 work was not touched.

### 2026-07-01 12:34:06 +08:00 - RBA-RBR corrected guard diagnostic manually launched on GPU0

- Reason: BVR child `1118197.542` was still running on protected hold GPU0 but appeared stable and low-memory; a hold-overlap GPU query before launch showed `CUDA_VISIBLE_DEVICES=0` and about `2441/24564 MiB` used.
- Action: cancelled only RBA public pending job `1132718 rba_guarddiag` after rechecking the exact record `1132718|rba_guarddiag|PENDING`; stopped only RBA watcher PID `1840205` to prevent duplicate launches.
- Launch: started child `1118197.560 rba_guard_g0` with logdir `/data/run01/sczc063/yuzibo/OpenTAD_RBA_RBR_GuardDiag_20260701_e6de60e9_bundle/logs/rba_rbr_guard_evaldiag_manual_parallel_holdg0_20260701_123213_+0800`.
- Startup evidence: launcher printed `CUDA_VISIBLE_DEVICES_INITIAL=0`; torch distributed init succeeded; training reached finite losses through `[000][00040/00199] Loss=2.5124`; grid audit reached `46` rows during startup.
- Boundary: parent hold `1118197` was not released/cancelled/replaced; GPU1/C3 was not touched. This remains `SHORT_DIAGNOSTIC_ONLY`; formal/full training and all metric/runtime/FLOPs/sparse-compute/deploy/paper claims remain locked.
