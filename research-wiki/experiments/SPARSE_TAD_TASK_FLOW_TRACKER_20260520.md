...
| --- | --- | --- | --- | --- | --- |
| MDL-Knot sampled_raw full-observation edge repair, `input_mdl_knot_dynamic_adapter_irregular_headv3*.py` | Input sampling handoff validator, sampled_raw audit metadata, profile instrumentation, diagnostics counters | `SHORT_DIAGNOSTIC_ONLY` Slurm job running; evidence branch synced | Local and remote py_compile/gates passed; remote focused pytest passed `60 passed in 204.98s`; final read-only subagent returned `PASS_SUBAGENT_FINAL_REVIEW_ONLY_FOR_SHORT_DIAGNOSTIC_ONLY`; clean restart gate passed; first finite losses observed | Remote commit `9183fb7`; active Slurm job `1132502` (`mdl_shortdiag`) running on `g0032`; clean logdir `/data/run01/sczc063/yuzibo/OpenTAD_MDLKnot_SampledRawFix_Precheck_20260701_f5fe3a7/logs/mdl_knot_shortdiag_sbatch_9183fb7_20260701_091642_+0800_torchrun_port29683_exclude_g0030`; iter 1 had one non-finite gradient skip, then iter 2-15 finite losses; current route state pushed and fast-forwarded with live-sync records on `codex/divergent-mdl-knot-status-410d499f-20260701`; no mAP/runtime/FLOPs/deploy/paper/sparse-compute claim | Let shortdiag finish or fail naturally; analyze slow `selector_and_structural_handoff`; formal/full long training remains locked |
...
### 2026-07-01 09:24:02 +08:00

- Route label: `DIVERGENT_INNOVATION_MDL_KNOT_DO_NOT_MERGE_WITH_C3`.
- Slurm child: `1132502` on `g0032`.
- Logdir: `/data/run01/sczc063/yuzibo/OpenTAD_MDLKnot_SampledRawFix_Precheck_20260701_f5fe3a7/logs/mdl_knot_shortdiag_sbatch_9183fb7_20260701_091642_+0800_torchrun_port29683_exclude_g0030`.
- First training evidence: iter 1 reported one skipped non-finite gradient in `module.rpn_head.reg_head.weight`; the diagnostic line showed finite uint8 inputs, `batch_irregular_valid_len=[768.0]`, mask valid count 384, and no non-finite input values.
- Subsequent finite losses: iter 2 `Loss=1.5851`, iter 3 `Loss=2.1502`, iter 4 `Loss=1.9394`, iter 5 `Loss=1.8892`, iter 6 `Loss=2.0425`, iter 7 `Loss=1.9420`; memory about `9750MB`.
- Speed evidence: sampled_raw `selector_and_structural_handoff` is still very slow for some videos, e.g. about `104.7s`, `98.9s`, `96.4s`, and `84.5s`; this confirms the route is launchable but still has a serious throughput issue.
- Current mAP evidence: none.
- Decision: let this bounded shortdiag continue or fail naturally and use it for stability/speed diagnosis only. Formal/full long training remains locked.

### 2026-07-01 09:31:39 +08:00

- Route label: `DIVERGENT_INNOVATION_MDL_KNOT_DO_NOT_MERGE_WITH_C3`.
- GitHub sync: ordinary push to existing branch `codex/divergent-mdl-knot-realdiag-20260630` was rejected as non-fast-forward; no force push or overwrite was attempted.
- Evidence branch sync succeeded: current local route-owned state was pushed to `codex/divergent-mdl-knot-status-410d499f-20260701` and then fast-forwarded with live-sync records after recording tracker/log evidence.
- GitHub URL: `https://github.com/yuzbo/pc-ot-mras-r-series-opentad/tree/codex/divergent-mdl-knot-status-410d499f-20260701`.
- Active Slurm job `1132502` remains `RUNNING` on `g0032`, not protected hold node `g0030`.
- New training evidence since the previous record: finite losses reached iter 15; no new Traceback/RuntimeError was observed in the inspected tail.
- Current mAP evidence: none.
- Decision: continue the bounded shortdiag for stability and speed evidence only. Formal/full long training, `tools/test.py`, evaluation result claims, runtime/FLOPs, deploy, paper, and sparse-compute claims remain locked.
