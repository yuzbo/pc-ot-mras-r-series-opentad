...
This route-owned tracker snapshot records only the PQR/QC V2 branch state in
`OpenTAD_C3PQRQCV2_Worktree_20260630`. The shared repository tracker remains
...
| --- | --- | --- | --- | --- | --- | --- |
| 2026-06-30 20:59:37 | `C3_PQR_RankCalV1_SparseIrregularQCV2` final review blocker fix | Fixed QC V2 point-geometry directional gap feature in `anchor_free_head.py`; added direct quality-head geometry test for non-uniform left/right gaps. No input sampler, Adapter, evaluator scoring/NMS, BH-SDC, DIVERGENT, GT/teacher/cache, remote, or Slurm change | Local blocker fix complete in owned worktree; no commit/push | Focused verification: config pytest `32 passed`; quality-head pytest default `1 passed, 8 skipped` due Windows torch DLL; `torch_1` quality-head pytest `1 passed, 8 skipped` due missing `nms_1d_cpu`; proposal/analyzer pytest `13 passed, 2 skipped`; QC V2 precheck/shortdiag validators PASS; py_compile PASS; `git diff --check` PASS with LF/CRLF warnings only | No SSH, remote sync, Slurm, `tools/train.py`, `tools/test.py`, GPU use, checkpoint, result JSON, mAP, parent-hold action, or route-quality evidence | Blocker is locally fixed. Remaining risk: direct torch-backed geometry test is skipped on this Windows environment and should run in Linux review/CI with `nms_1d_cpu` available before deployment/sync/training |
| 2026-06-30 20:42:02 | `C3_PQR_RankCalV1_SparseIrregularQCV2` full local implementation in owned worktree | Detector head QC V2 quality/rank branch now consumes deploy-visible sparse/irregular geometry; richer proposal diagnostics/analyzer state JSON; locked fulltrain-candidate config; validator/tests. No input sampler, Adapter, evaluator scoring/NMS, BH-SDC, DIVERGENT, GT/teacher/cache, remote, or Slurm change | Local code implementation complete in `OpenTAD_C3PQRQCV2_Worktree_20260630`; no commit/push yet; Pro and subagent review gates incomplete due local transport/tool failure | Validator PASS for QC V2 precheck/shortdiag/fulltrain-candidate; `py_compile` PASS; config pytest `32 passed`; proposal/analyzer pytest `13 passed, 2 skipped`; quality-head pytest default `1 passed, 7 skipped` due Windows torch DLL; `torch_1` quality-head pytest `1 passed, 7 skipped` due missing `nms_1d_cpu`; `git diff --check` PASS with LF/CRLF warnings only; Oracle Pro unavailable (`OPENAI_API_KEY` missing, Rosetta CDP refused); `claude_review` twice failed with `Claude CLI did not return JSON output` | No SSH, remote sync, Slurm, `tools/train.py`, `tools/test.py`, checkpoint, result JSON, mAP, GPU use, parent-hold action, or route-quality evidence. Test-time GT/teacher/cache remains rejected; train-only GT quality target remains supervision only | Main process may inspect/commit local code. Before any deployment, sync, remote PRECHECK, diagnostic training, long run, or claim, complete required Pro review and read-only subagent final review in a functioning review channel |
...
## 2026-06-30 14:52:34 +08:00 User-Reaffirmed GPU1-Only Rule

- User instruction: C3/PQR/CADF mainline training must keep using GPU1, because
  GPU0 is reserved for innovation/divergent experiment processes.
- Operational decision: treat GPU1-only as a hard route-ownership rule for all
  future C3/PQR/CADF mainline training, diagnostic training, short smoke,
  bounded `tools/train.py`, and Slurm child launches.
- Fallback rule: do not use GPU0 if GPU1 is busy. Wait, queue, or report the
  blocker instead.
- Current implementation support: the QC V2 shortdiag launcher requires
