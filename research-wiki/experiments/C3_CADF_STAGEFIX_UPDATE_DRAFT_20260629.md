...
## Local CADF Loss-Select V2 Candidate - 2026-06-30 20:52 +0800

- Scope: local-only work in `OpenTAD_C3CADFLossSelect_Worktree_20260630` on branch `codex/c3-cadf-loss-select-v2-20260630`, base `c333157d390d94e4ca8390f7c0f635799d2826d8`. No SSH, Slurm, remote sync, remote training, GPU0 use, BH-SDC, DIVERGENT, evaluator/postprocess, ranking, checkpoint cleanup, or mAP claim was performed.
- User-provided stability event absorbed as a hard local requirement: remote GPU1 fastfix formal child `1118197.500 cadf_ff_full_g1` reportedly reached sustained `Loss=nan` from epoch 4 through epoch 9, including `loss_c3_actionness/cls_loss/reg_loss`, with no OOM/Traceback and no real Evaluation/Average-mAP. This local candidate treats that as a stability gate, not as a final route metric.
- Selector implementation: added CADF loss-select V2 auxiliary acquisition objective in `opentad/models/selectors/c3_indirect_frame_selector.py`. It does not replace detector loss. Components include density smoothness, local density cap, large-gap expected penalty, collapse/repeat penalty, and deployable target-density KL from scout actionness/uncertainty/change/action-context. Optional GT target blending is train-only; test-time selection remains based on scout/actionness/uncertainty/change and never uses GT, teacher, or cache inputs.
- Numeric stability: selector logits/density/probability paths now use fp32 finite guards. Sigmoid/log/softmax/BCE/ST paths use `torch.nan_to_num` plus clamp; robust normalization uses finite-only fp32 values to preserve row-scale robustness before normalized clipping; density is sanitized and renormalized with valid-mask fallback. Actionness BCE, density auxiliary losses, acquisition logits, softmax density, and ST local softmax are fp32 islands.
- Train-engine guard: added `solver.nonfinite_loss_guard` support in `opentad/cores/train_engine.py` and `tools/train.py`. When enabled, non-finite `cost` or loss components skip backward/optimizer/scheduler/EMA for that iteration and log the offending names; configured threshold can fail closed with `RuntimeError` before `optimizer.step` pollutes parameters. CADF loss-select V2 configs set `enabled=True, max_skips=0, max_consecutive_skips=0`.
- Diagnostics: selected-index metadata now includes density histogram, duplicate count, fill fraction, uncertainty/change selected fraction, distribution-target selected fraction/top positions, existing max-gap/mean-gap/repair stats, and loss component metadata for training.
- New configs: `c3_cadf_densitymesh_original_adatad_32px_loss_select_v2_precheck.py`, `c3_cadf_densitymesh_original_adatad_32px_loss_select_v2_shortdiag.py`, and `c3_cadf_densitymesh_original_adatad_32px_loss_select_v2_formal_candidate_locked.py`. Precheck and shortdiag are local diagnostic candidates with AMP/fp16/EMA disabled. Formal is full-train-shaped but fail-closed pending user unlock and evidence; it preserves fast CPU selection and bounded diagnostics.
- New GPU1-only launchers: `scripts/run_c3_cadf_loss_select_v2_precheck_gpu1.sh`, `scripts/run_c3_cadf_loss_select_v2_shortdiag_gpu1.sh`, and `scripts/run_c3_cadf_loss_select_v2_formal_candidate_locked_gpu1.sh`. All require `CUDA_VISIBLE_DEVICES=1`; formal additionally requires `CADF_LOSS_SELECT_V2_FORMAL_UNLOCK=CONFIRMED` and an evidence file before reaching `tools/train.py`.
- Validator update: `tools/validate_c3_indirect_clean_config.py` now enforces loss-select V2 deploy-time input contract, leakage lock, nonfinite-loss guard, AMP/fp16/EMA off until NaN gate is cleared, fast CPU selection preservation, and formal fail-closed user unlock.
- Tests added/updated: selector loss and diagnostics tests in `tests/test_c3_cadf_densitymesh_selector.py`; config/launcher/validator tests in `tests/test_c3_cadf_densitymesh_config.py`; train-engine nonfinite guard tests in `tests/test_c3_cadf_train_engine_guard.py`; signature test in `tests/test_train_engine_max_train_iters.py`.
- Verification:
  - `conda run -n torch_1 python -m pytest tests\test_c3_cadf_densitymesh_selector.py tests\test_c3_cadf_densitymesh_config.py tests\test_c3_cadf_train_engine_guard.py tests\test_train_engine_max_train_iters.py -q`: `91 passed in 56.08s`.
  - `python -m py_compile opentad\models\selectors\c3_indirect_frame_selector.py opentad\cores\train_engine.py tools\train.py tools\validate_c3_indirect_clean_config.py`: pass.
  - `conda run -n torch_1 python tools\validate_c3_indirect_clean_config.py configs\adatad\thumos\c3_cadf_densitymesh_original_adatad_32px_loss_select_v2_precheck.py`: pass.
  - `conda run -n torch_1 python tools\validate_c3_indirect_clean_config.py configs\adatad\thumos\c3_cadf_densitymesh_original_adatad_32px_loss_select_v2_shortdiag.py`: pass.
  - `conda run -n torch_1 python tools\validate_c3_indirect_clean_config.py configs\adatad\thumos\c3_cadf_densitymesh_original_adatad_32px_loss_select_v2_formal_candidate_locked.py`: pass.
  - `git diff --check`: exit 0; Windows LF/CRLF warnings only.
- Decision: local implementation candidate is complete for code/config/test purposes, but GPU validation remains locked. No shortdiag/fulltrain/mAP result exists for this loss-select V2 candidate, and short training must not be used as final route judgment.
