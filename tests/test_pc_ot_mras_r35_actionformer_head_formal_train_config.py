...
CONFIG = (
    ...
    / "adatad"
    / "thumos"
    / "ctf_bdi_pc_ot_mras_r35_actionformer_head_formal_train_candidate.py"
)
LAUNCHER = ROOT / "scripts" / "run_ctf_bdi_pc_ot_mras_r35_actionformer_head_formal_train_n16r4.sbatch"
GUARD_PATH = ROOT / "opentad" / "utils" / "training_guard.py"


def _load_guard():
    spec = importlib.util.spec_from_file_location("training_guard_for_r35_actionformer_head_test", GUARD_PATH)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def test_r35_actionformer_head_config_is_parseable_and_gate_bound(tmp_path, monkeypatch):
    ...
    gate = cfg.r35_pc_ot_mras_actionformer_head_gate
    assert gate.route == "CTF-BDI/PC-OT-MRAS"
    assert gate.stage == "R35_actionformer_head_formal_train_candidate"
    ...
    assert gate.entrypoint_gate_context.allowed_decisions == ("ALLOW_R35_ACTIONFORMER_HEAD_FORMAL_TRAIN",)
    ...
    assert cfg.model.neck.source_feature_level == 0
    assert list(cfg.model.neck.output_strides) == [1, 2, 4, 8, 16, 32]

    head = cfg.model.rpn_head
    assert head.type == "ActionFormerHead"
    assert "temporal_grid" not in head
    assert "area_head" not in head
    assert head.prior_generator.type == "PointGenerator"
    assert list(head.prior_generator.strides) == [1, 2, 4, 8, 16, 32]
    assert [tuple(item) for item in head.prior_generator.regression_range] == [
        (0, 4),
        (4, 8),
        (8, 16),
        (16, 32),
    ...
    gate_json = tmp_path / "r35_gate.json"
    gate_json.write_text(
        json.dumps(
            {
                "decision": "ALLOW_R35_ACTIONFORMER_HEAD_FORMAL_TRAIN",
...
def test_r35_actionformer_head_config_has_no_test_time_shortcut_tokens():
    mmengine_config = pytest.importorskip("mmengine.config")
    cfg = mmengine_config.Config.fromfile(str(CONFIG))
...
def test_r35_actionformer_head_launcher_is_clean_repo_gate_bound_and_fail_closed():
    text = LAUNCHER.read_text(encoding="utf-8")

    assert "#SBATCH --gpus=1" in text
    assert "#SBATCH -J pcot_r35afh" in text
    ...
    assert "ctf_bdi_pc_ot_mras_r35_actionformer_head_formal_train_candidate.py" in text
    assert 'PRECHECK_ONLY="${PRECHECK_ONLY:-1}"' in text
    assert 'ALLOW_R35_ACTIONFORMER_HEAD_FORMAL_TRAIN="${ALLOW_R35_ACTIONFORMER_HEAD_FORMAL_TRAIN:-0}"' in text
    assert "PRECHECK_ONLY=0 requires ALLOW_R35_ACTIONFORMER_HEAD_FORMAL_TRAIN=1" in text
    assert "PRECHECK_ONLY=1 must not set ALLOW_R35_ACTIONFORMER_HEAD_FORMAL_TRAIN=1" in text
    assert "R35_ACTIONFORMER_HEAD_FORMAL_GATE_JSON" in text
    assert "R35 execution gate active manifest sha256 mismatch" in text
    assert "R35 execution gate resolved config sha256 mismatch" in text
    ...
    assert "R35_ACTIONFORMER_HEAD_FORMAL_PRECHECK_ONLY_PASS_NO_TRAIN" in text
    assert "R35_ACTIONFORMER_HEAD_FORMAL_TRAIN_PASS_NO_DIRECT_TEST_NO_CLAIMS" in text
