...
def test_actionformer_calls_boundary_selector_before_backbone_on_videomae_6d_layout():
    ...
    assert train_metas[0]["boundary_microscope_true_observation_positions"] == [float(pos) for pos in selected]
    assert train_metas[0]["boundary_microscope_detector_input_positions"] == [float(pos) for pos in full_indices]
