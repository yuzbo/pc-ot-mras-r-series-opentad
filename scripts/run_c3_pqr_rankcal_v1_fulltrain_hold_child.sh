...
CONFIG_REL="configs/adatad/thumos/c3_indirect_original_adatad_32px_a_pqr_rankcal_v1_fulltrain.py"
CONFIG="$ROOT/$CONFIG_REL"
STAMP="${PQR_STAMP:-$(date +%Y%m%d_%H%M%S_%z)}"
RUN_NAME="${PQR_RUN_NAME:-pqr_rankcal_v1_fulltrain_${STAMP}}"
...

{
  ...
  echo "formal_fulltrain=true"
  echo "user_override_fulltrain=true"
  ...
  export MASTER_PORT="${MASTER_PORT:-30937}"
  ...
  python tools/analyze_c3_pqr_rankcal_proposals.py \
    ...
    --output "$RUN_DIR/pqr_score_iou_rank_proposal_diagnostic_final.json" \
    --records-csv "$RUN_DIR/pqr_score_iou_rank_proposal_records_final.csv" || true
