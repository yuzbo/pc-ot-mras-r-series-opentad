...
QUEUE_LOG="$LOG_DIR/adapter_actionformer_regloss_driver.log"
GPU_ID="${GPU_ID:-0}"
...
assert abs(float(cfg.model.rpn_head.loss_weight) - 1.5) < 1e-9, cfg.model.rpn_head
...
print("loss_weight=", cfg.model.rpn_head.loss_weight)
...
log_msg "adapter ActionFormer regloss queue start"
df -h /root/autodl-tmp | tee -a "$QUEUE_LOG"
check_cfg

if [[ "$CHECK_ONLY" == "1" ]]; then
  log_msg "adapter ActionFormer regloss check-only complete"
  exit 0
fi

wait_for_gpu
run_exp
log_msg "adapter ActionFormer regloss queue complete"
