...
class NativePhysicalMultiScaleHeadV2(nn.Module):
    def __init__(
        self,
        num_classes,
        in_channels,
    ...
    def _validate_inputs(self, feat_list, mask_list, temporal_grid_list):
        if temporal_grid_list is None:
            raise ValueError("NativePhysicalMultiScaleHead requires temporal_grid_list.")
        if not (len(feat_list) == len(mask_list) == len(temporal_grid_list)):
            raise ValueError(
                "feat_list, mask_list, and temporal_grid_list must have the same length, "
                f"got {len(feat_list)}, {len(mask_list)}, {len(temporal_grid_list)}."
            )
        if len(feat_list) == 0:
            raise ValueError("NativePhysicalMultiScaleHead received an empty feat_list.")
        if len(feat_list) != len(self.scale):
            raise ValueError(
                f"Expected {len(self.scale)} feature levels from prior_generator.strides, got {len(feat_list)}."
            )

        for level_idx, (feat, mask, grid) in enumerate(zip(feat_list, mask_list, temporal_grid_list)):
            if feat.dim() != 3:
                raise ValueError(f"feat_list[{level_idx}] must have shape [B, C, T], got {tuple(feat.shape)}.")
            if mask.dim() != 2:
                raise ValueError(f"mask_list[{level_idx}] must have shape [B, T], got {tuple(mask.shape)}.")
            if feat.shape[0] != mask.shape[0] or feat.shape[-1] != mask.shape[-1]:
                raise ValueError(
                    f"Feature/mask shape mismatch at level {level_idx}: "
                    f"feat={tuple(feat.shape)}, mask={tuple(mask.shape)}."
                )
            for key in ("center", "valid_mask", "cell_left", "cell_right"):
                if key not in grid:
                    raise KeyError(f"temporal_grid_list[{level_idx}] is missing required key '{key}'.")
                if grid[key].shape != mask.shape:
                    raise ValueError(
                        f"temporal_grid_list[{level_idx}]['{key}'] must match mask shape {tuple(mask.shape)}, "
                        f"got {tuple(grid[key].shape)}."
                    )

    def _validate_points(self, points, feat_list):
        if len(points) != len(feat_list):
            raise ValueError(f"prior_generator returned {len(points)} point levels, expected {len(feat_list)}.")
        for level_idx, (point, feat) in enumerate(zip(points, feat_list)):
            if point.dim() != 3 or point.shape[-1] < 5:
                raise ValueError(
                    f"points[{level_idx}] must have shape [B, T, >=5], got {tuple(point.shape)}."
                )
            if point.shape[0] != feat.shape[0] or point.shape[1] != feat.shape[-1]:
                raise ValueError(
                    f"Point/feature shape mismatch at level {level_idx}: "
                    f"points={tuple(point.shape)}, feat={tuple(feat.shape)}."
                )
    ...
    def forward_train(self, feat_list, mask_list, temporal_grid_list=None, gt_segments=None, gt_labels=None, **kwargs):
        self._validate_inputs(feat_list, mask_list, temporal_grid_list)
        ...
        self._validate_points(points, feat_list)
    ...
    def get_refined_proposals(self, points, reg_pred):
        ...
        if point_tensor.shape[:2] != reg_tensor.shape[:2]:
            raise ValueError(
                f"Point/regression shape mismatch: points={tuple(point_tensor.shape)}, "
                f"reg={tuple(reg_tensor.shape)}."
            )
    ...
    def _tensor_stats(self, tensor, prefix):
        detached = tensor.detach()
        finite = torch.isfinite(detached)
        numel = int(detached.numel())
        finite_count = int(finite.sum().item())
        state = {
            f"{prefix}_shape": tuple(detached.shape),
            f"{prefix}_dtype": str(detached.dtype),
            f"{prefix}_numel": numel,
            f"{prefix}_finite_count": finite_count,
            f"{prefix}_nonfinite_count": int(numel - finite_count),
        }
        if finite_count > 0:
            finite_values = detached[finite]
            if not torch.is_floating_point(finite_values):
            ...
            state.update(
                {
                    ...
                    f"{prefix}_std": float(finite_values.std(unbiased=False).item()),
                    f"{prefix}_absmax": float(finite_values.abs().max().item()),
                }
            )
        return state
    ...
    def losses(self, cls_pred, reg_pred, mask_list, temporal_grid_list, points, gt_segments, gt_labels):
        ...
        if self.debug_enabled:
            ...
            debug_state["native_ms_head_cls_loss_weight"] = float(self.cls_loss_weight)
            debug_state["native_ms_head_reg_loss_weight"] = float(self.reg_loss_weight)
            debug_state["native_ms_head_loss_weight"] = float(self.loss_weight)
            for level_idx, (reg_level, (start_idx, end_idx)) in enumerate(zip(reg_pred, level_offsets)):
                ...
                debug_state[f"native_ms_head_level{level_idx}_positive_ratio"] = float(
                    level_pos.sum().item() / max(int(level_valid.sum().item()), 1)
                )
                debug_state.update(self._tensor_stats(reg_level, f"native_ms_head_level{level_idx}_reg_pred"))
    ...
    def prepare_targets(self, points, mask_list, temporal_grid_list, gt_segments, gt_labels):
        ...
        if concat_points.shape[0] != len(gt_segments) or concat_points.shape[0] != len(gt_labels):
            raise ValueError(
                "Batch size mismatch between points and targets: "
                f"points batch={concat_points.shape[0]}, gt_segments={len(gt_segments)}, gt_labels={len(gt_labels)}."
            )
        ...
        for point, valid, gt_segment, gt_label in zip(concat_points, valid_mask, gt_segments, gt_labels):
            ...
            if valid_centers.numel() > 0:
                debug_state.setdefault("native_ms_head_center_min_per_sample", []).append(float(valid_centers.min().item()))
                debug_state.setdefault("native_ms_head_center_max_per_sample", []).append(float(valid_centers.max().item()))
            else:
                debug_state.setdefault("native_ms_head_center_min_per_sample", []).append(None)
                debug_state.setdefault("native_ms_head_center_max_per_sample", []).append(None)

            for level_idx, (start_idx, end_idx) in enumerate(level_offsets):
                level_valid = valid[start_idx:end_idx]
                level_centers = centers[start_idx:end_idx][level_valid]
                debug_state.setdefault(f"native_ms_head_level{level_idx}_valid_per_sample", []).append(
                    int(level_valid.sum().item())
                )
                if level_centers.numel() > 0:
                    debug_state.setdefault(f"native_ms_head_level{level_idx}_center_min_per_sample", []).append(
                        float(level_centers.min().item())
                    )
                    debug_state.setdefault(f"native_ms_head_level{level_idx}_center_max_per_sample", []).append(
                        float(level_centers.max().item())
                    )
                else:
                    debug_state.setdefault(f"native_ms_head_level{level_idx}_center_min_per_sample", []).append(None)
                    debug_state.setdefault(f"native_ms_head_level{level_idx}_center_max_per_sample", []).append(None)
            ...
            if num_gts == 0:
                ...
                debug_state.setdefault("native_ms_head_reg_target_left_mean_per_sample", []).append(0.0)
                debug_state.setdefault("native_ms_head_reg_target_right_mean_per_sample", []).append(0.0)
                debug_state.setdefault("native_ms_head_reg_target_absmax_per_sample", []).append(0.0)
                for level_idx in range(len(level_offsets)):
                    debug_state.setdefault(f"native_ms_head_level{level_idx}_center_hits_per_sample", []).append(0)
                    debug_state.setdefault(f"native_ms_head_level{level_idx}_range_hits_per_sample", []).append(0)
                    debug_state.setdefault(f"native_ms_head_level{level_idx}_matched_hits_per_sample", []).append(0)
                    debug_state.setdefault(f"native_ms_head_level{level_idx}_pos_per_sample", []).append(0)
                    debug_state.setdefault(f"native_ms_head_level{level_idx}_reg_target_mean_per_sample", []).append(0.0)
                    debug_state.setdefault(f"native_ms_head_level{level_idx}_reg_target_absmax_per_sample", []).append(0.0)
                continue
            ...
            if finite_assignment.any():
                assigned_spans = (assigned_segments[finite_assignment, 1] - assigned_segments[finite_assignment, 0]).clamp_min(
                    0.0
                )
            ...
            physical_reg_targets = torch.stack(
                [
                    centers - assigned_segments[:, 0],
                    assigned_segments[:, 1] - centers,
                ],
                dim=-1,
            )
            physical_reg_targets = torch.where(
                finite_assignment[:, None],
                physical_reg_targets,
                physical_reg_targets.new_zeros(physical_reg_targets.shape),
            )

            if positive_mask.any():
                pos_reg = physical_reg_targets[positive_mask]
                debug_state.setdefault("native_ms_head_reg_target_left_mean_per_sample", []).append(
                    float(pos_reg[:, 0].mean().item())
                )
                debug_state.setdefault("native_ms_head_reg_target_right_mean_per_sample", []).append(
                    float(pos_reg[:, 1].mean().item())
                )
                debug_state.setdefault("native_ms_head_reg_target_absmax_per_sample", []).append(
            ...
            for level_idx, (start_idx, end_idx) in enumerate(level_offsets):
                level_center_hits = center_hits[start_idx:end_idx]
                level_range_hits = range_hits[start_idx:end_idx]
                level_matched_hits = matched_hits[start_idx:end_idx]
                level_pos = positive_mask[start_idx:end_idx]
                debug_state.setdefault(f"native_ms_head_level{level_idx}_center_hits_per_sample", []).append(
                    int(level_center_hits.sum().item())
                )
                debug_state.setdefault(f"native_ms_head_level{level_idx}_range_hits_per_sample", []).append(
                    int(level_range_hits.sum().item())
                )
                debug_state.setdefault(f"native_ms_head_level{level_idx}_matched_hits_per_sample", []).append(
                    int(level_matched_hits.sum().item())
                )
                debug_state.setdefault(f"native_ms_head_level{level_idx}_pos_per_sample", []).append(
                    int(level_pos.sum().item())
                )
                if level_pos.any():
                    level_reg = physical_reg_targets[start_idx:end_idx][level_pos]
                    debug_state.setdefault(f"native_ms_head_level{level_idx}_reg_target_mean_per_sample", []).append(
                        float(level_reg.mean().item())
                    )
                    debug_state.setdefault(f"native_ms_head_level{level_idx}_reg_target_absmax_per_sample", []).append(
