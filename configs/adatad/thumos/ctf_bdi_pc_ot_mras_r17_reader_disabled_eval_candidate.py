...
# R17 completed-checkpoint reader-disabled evaluation candidate.
# This keeps the trained detector/head/checkpoint surface but bypasses the
# learned reader at eval time through a deterministic exact-uniform payload.

r17_pc_ot_mras_post_train_eval_gate = None

r17_pc_ot_mras_reader_disabled_eval_gate = dict(
    route="CTF-BDI/PC-OT-MRAS",
    stage="R17_reader_disabled_eval_candidate",
    reviewed_predecessor="R17_post_train_eval_candidate",
    default_off=True,
    explicit_config_opt_in=True,
    reader_disabled_eval_candidate=True,
    ...
    entrypoint_gate_context=dict(
        ...
        allowed_decisions=("ALLOW_R17_READER_DISABLED_EVAL",),
        forbidden_true_keys=(
            "tools_train",
            "direct_tools_train",
    ...
    allowed_checks=(
        ...
        "reader_disabled_exact_uniform_eval_override",
...
model = dict(
    pc_ot_mras_reader_eval_override=dict(
        enabled=True,
        mode="exact_uniform",
        num_slots=384,
    ),
...
work_dir = "exps/thumos/adatad/ctf_bdi_pc_ot_mras_r17_reader_disabled_eval_candidate"
