...
def _allowed_payload_keys():
    return set(CONTROL_KEYS) | set(REQUIRED_TRUE_KEYS) | set(FORBIDDEN_TRUE_KEYS)


def _is_unknown_gate_key(key):
    key_text = str(key)
    if key_text in _allowed_payload_keys():
        return False
    if key_text in HARMLESS_METADATA_KEYS:
        return False
    return True


def validate_gate_payload(payload, active_manifest_sha256, resolved_config_sha256):
    """Validate the future P2QR bounded short-smoke permission payload."""
    if payload.get("decision") != ALLOWED_DECISION:
        raise ValueError(f"P2QR gate decision is not allowed: {payload.get('decision')}")

    if payload.get("route") not in (None, "CTF-BDI/PC-OT-MRAS"):
        raise ValueError(f"P2QR gate route mismatch: {payload.get('route')}")
    ...
    if not expected_manifest:
        raise ValueError("P2QR gate must bind active_sha256_manifest_sha256")
    if expected_manifest != active_manifest_sha256:
        raise ValueError(
            "P2QR gate active manifest sha256 mismatch: "
    ...
    if not expected_resolved:
        raise ValueError("P2QR gate must bind resolved_config_sha256")
    if expected_resolved != resolved_config_sha256:
        raise ValueError(
            "P2QR gate resolved config sha256 mismatch: "
            f"expected={expected_resolved} actual={resolved_config_sha256}"
        )

    if payload.get("max_train_iters") != 2:
        raise ValueError(f"P2QR gate must preserve max_train_iters=2: {payload.get('max_train_iters')}")

    for key in REQUIRED_TRUE_KEYS:
        if payload.get(key) is not True:
            raise ValueError(f"P2QR gate must set {key}=true for the bounded Slurm short smoke")

    for key in FORBIDDEN_TRUE_KEYS:
        if key in payload and payload[key] is not False:
            raise ValueError(f"P2QR gate must keep {key}=false/absent; got {payload[key]!r}")

    for key in payload:
        if _is_unknown_gate_key(key):
            raise ValueError(f"P2QR gate contains unknown or unallowlisted key: {key}")
...
def main():
    ...
    print("P2QR_SHORT_SMOKE_GATE_VALIDATION_PASS")
