...
ALLOWED_DECISION = "ALLOW_PC_OT_MRAS_FRONTEND_JOINT_E2E_SOFT_TRANSPORT_TRAIN"
...
def _require_exact(payload, key, expected):
    if payload.get(key) != expected:
        raise ValueError(f"frontend joint E2E gate must preserve {key}={expected}: {payload.get(key)}")


def _require_sha(payload, key, expected):
    value = payload.get(key)
    if not value:
        raise ValueError(f"frontend joint E2E gate must bind {key}")
    if value != expected:
        raise ValueError(f"frontend joint E2E gate {key} mismatch: expected={expected} actual={value}")
...
def validate_gate_payload(
    ...
    if payload.get("decision") != ALLOWED_DECISION:
        raise ValueError(f"frontend joint E2E gate decision is not allowed: {payload.get('decision')}")

    if payload.get("route") != ALLOWED_ROUTE:
        raise ValueError(f"frontend joint E2E gate route mismatch: {payload.get('route')}")
    ...
    if not expected_manifest:
        raise ValueError("frontend joint E2E gate must bind active_sha256_manifest_sha256")
    if expected_manifest != active_manifest_sha256:
        raise ValueError(
            "frontend joint E2E gate active manifest sha256 mismatch: "
    ...
    if not expected_resolved:
        raise ValueError("frontend joint E2E gate must bind resolved_config_sha256")
    if expected_resolved != resolved_config_sha256:
        raise ValueError(
            "frontend joint E2E gate resolved config sha256 mismatch: "
    ...
    for key in REQUIRED_TRUE_KEYS:
        if payload.get(key) is not True:
            raise ValueError(f"frontend joint E2E gate must set {key}=true")

    for key in FORBIDDEN_TRUE_KEYS:
        if key in payload and payload[key] is not False:
            raise ValueError(f"frontend joint E2E gate must keep {key}=false/absent; got {payload[key]!r}")

    for key in payload:
        if key not in _allowed_payload_keys():
            raise ValueError(f"frontend joint E2E gate contains unknown or unallowlisted key: {key}")
...
def validate_gate_file(
    ...
    if not gate_path.is_file():
        raise ValueError(f"missing frontend joint E2E gate JSON: {gate_json}")
    actual_sha256 = sha256_file(gate_path)
    if actual_sha256 != gate_sha256:
        raise ValueError(
            f"frontend joint E2E gate sha256 mismatch: expected={gate_sha256} actual={actual_sha256}"
        )
...
def main():
    parser = argparse.ArgumentParser(description="Validate a frontend joint E2E soft-transport train gate.")
    ...
    print("PC_OT_MRAS_FRONTEND_JOINT_E2E_GATE_VALIDATION_PASS")
